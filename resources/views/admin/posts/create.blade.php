...
@push('after-style')
    ...
    <style>
        ...
        .img-btn-remove { background: #fef2f2; color: #ef4444; }
        .img-btn-remove:hover { background: #fee2e2; }
        ...
        .f-btn-schedule { background: #2563eb; color: white; box-shadow: 0 8px 24px rgba(37,99,235,0.18); }
        .f-btn-schedule:hover { background: #1d4ed8; transform: translateY(-3px); }
...
<div class="form-page">
    ...
    <p class="page-subtitle">Isi form di bawah untuk membuat berita baru</p>
    ...
    <form id="formBerita" action="{{ route('posts.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="form-card">
            ...
            <div class="field">
                ...
                <input type="text" name="judul" class="field-input" required autocomplete="off" placeholder="Tulis judul berita yang menarik..." value="{{ old('judul') }}">
            </div>
            <div class="field-row">
                <div class="field">
                    <label class="field-label">Kategori <span class="req">*</span></label>
                    <input type="text" name="tagline" class="field-input" autocomplete="off" placeholder="Contoh: Teknologi, Bisnis..." value="{{ old('tagline') }}">
                </div>
                <div class="field">
                    <label class="field-label">Penulis <span class="req">*</span></label>
                    <input type="text" name="penulis" class="field-input" required autocomplete="off" placeholder="Nama penulis..." value="{{ old('penulis') }}">
                </div>
            </div>
        </div>
        ...
        <div class="form-card">
            ...
            <div class="tags-list" id="tagsContainer">
                <div class="tag-row">
                    <input type="text" name="tags[]" class="field-input" autocomplete="off" placeholder="Masukkan kata kunci...">
            ...
            <div class="field">
                ...
                <div id="previewBox" class="img-preview" style="display:none;">
                    ...
                    <div class="img-preview-btns">
                        <button type="button" class="img-btn-change" id="btnChangeImage">Ganti</button>
                        <button type="button" class="img-btn-remove" id="btnRemoveImage">Hapus</button>
                    </div>
                </div>
            </div>
            <div class="field">
                <label class="field-label">Caption Foto <span class="req">*</span></label>
                <input type="text" name="caption" class="field-input" autocomplete="off" placeholder="Deskripsi singkat gambar..." value="{{ old('caption') }}">
            </div>
        </div>

        <!-- SCHEDULE (hidden by default) -->
        <div class="form-card" id="scheduleSection" style="display:none;">
            <div class="form-card-title">Jadwal Publikasi</div>
            <div class="field">
                <label class="field-label">Tanggal & Waktu <span class="req">*</span></label>
                <input type="datetime-local" name="scheduled_at" id="scheduledAt" class="field-input" min="{{ now()->format('Y-m-d\TH:i') }}">
            </div>
        </div>
        ...
        <div class="form-footer">
            ...
            <button type="button" id="btnSchedule" class="f-btn f-btn-schedule">Schedule</button>
            <button type="submit" class="f-btn f-btn-primary">Unggah Berita</button>
        </div>
...
<script>
    ...
    btnRemoveImage.addEventListener('click', () => { inputGambar.value = ''; previewBox.style.display = 'none'; uploadBox.style.display = 'block'; });
    ...
    function showPreview(file) {
        ...
        reader.onload = e => { progressBar.style.width = '100%'; progressText.textContent = 'Selesai!'; setTimeout(() => { progressBox.style.display = 'none'; previewImg.src = e.target.result; previewName.textContent = file.name; previewBox.style.display = 'block'; }, 400); };
    ...
    btnSchedule.addEventListener('click', function() {
        ...
        if (isScheduleMode) { scheduleSection.style.display = 'block'; postStatus.value = 'future'; scheduledAt.setAttribute('required','required'); btnSchedule.textContent = 'Batal Schedule'; btnSchedule.style.background = '#ef4444'; btnSchedule.style.boxShadow = '0 8px 24px rgba(239,68,68,0.18)'; }
        else { scheduleSection.style.display = 'none'; postStatus.value = 'publish'; scheduledAt.removeAttribute('required'); scheduledAt.value = ''; btnSchedule.textContent = 'Schedule'; btnSchedule.style.background = '#2563eb'; btnSchedule.style.boxShadow = '0 8px 24px rgba(37,99,235,0.18)'; }
    });
