                ...
                <div class="form-group form-full">
                    ...
                    <div id="previewBox" class="preview-box" style="display:none;">
                        ...
                        <div class="preview-actions" style="justify-content:center;">
                            ...
                            <button type="button" class="btn-remove" id="btnRemoveImage">Hapus</button>
                        </div>
                    </div>
                </div>
...
<script>
    ...
    (function guardTrixFocus() {
        ...
        document.addEventListener('pointerdown', function(e) {
            ...
            if (e.target.closest('trix-editor, .trix-wrapper, trix-toolbar')) {
                allowTrixFocusUntil = Date.now() + 800;
            }
        }, true);
        ...
        function setupEditor(editor) {
            ...
            const allowDirectFocus = function() {
                allowTrixFocusUntil = Date.now() + 800;
            };
            ...
            editor.addEventListener('pointerdown', allowDirectFocus, true);
            editor.addEventListener('keydown', allowDirectFocus, true);

            editor.addEventListener('focus', function() {
                if (Date.now() < allowTrixFocusUntil || editor.contains(pointerTarget)) {
                    return;
                }
                ...
                requestAnimationFrame(function() {
                    ...
                    const activeField = pointerTarget?.closest?.('input:not([type="hidden"]), textarea, select');
                    if (activeField) {
    ...
    uploadBox.addEventListener('drop', e => {
        ...
        if (file && validateImage(file)) { inputGambar.files = e.dataTransfer.files; showPreview(file); }
    });
    ...
    btnRemoveImage.addEventListener('click', () => {
        ...
        previewName.textContent = 'Gambar dipilih';
        previewBox.style.display = 'none';
        uploadBox.style.display = 'block';
    });
    inputGambar.addEventListener('change', e => { const file = e.target.files[0]; if (file && validateImage(file)) showPreview(file); });
